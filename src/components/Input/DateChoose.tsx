...
export default function CustomDatePicker(props: DateChooseProps) {
	...
	return (
		<div className="relative w-72" ref={ref}>
			...
			>
				<span
					className={`text-sm leading-6 font-normal ${selected?.from ? "text-[#1A1A1A]" : "text-[#999999]"}`}
				>
					{selected?.from
						? selected.to
			...
			<div className="pointer-events-none absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-1">
				{selected && (
					<button
						type="button"
						onClick={handleClear}
						className="pointer-events-auto cursor-grab flex items-center"
					>
						<Image src={Cancel} alt="cancle" width={20} height={20} />
					</button>
				)}
