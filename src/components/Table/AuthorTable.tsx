...
import { AuthorData, AuthorTableProps } from "@/types/Table/AuthorTable";
...
const AuthorTable = (props: AuthorTableProps) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return props.AuthorData.filter((item) =>
    ...
    <div className="p-6">
          ...
          <thead className="bg-[#F9F9F9]">
            <tr>
              ...
              {[
                ...
                >
                  {label}{renderSortArrow(key as keyof AuthorData)}
                </th>
              ))}
          ...
          {paginatedData.length === 0 ? (
            ...
            <tbody>
              {paginatedData.map((author) => (
                <tr key={author.name} className={`border-b-[1px] border-[#F1F1F1] text-sm leading-6 font-normal text-[#1A1A1A] ${selectedIds.has(author.name) ? "bg-[#F3F6F7] border-[1px] border-[#F1F1F1]" : ""
                  }`}>
      ...
      <div className="flex items-center mt-4 gap-2">
        <p className="text-[#A7A7A7] font-normal text-sm leading-[22px]">
          顯示 {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredData.length)} 筆資料，共 {filteredData.length} 筆
        </p>
        ...
        >
          {[5, 10, 15, 20, 50, 100].map((num) => (
            <option key={num} value={num} className="text-[#040404] font-normal text-sm leading-[22px]">{num} 筆</option>
          ))}
        </select>
